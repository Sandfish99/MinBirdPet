...
> 简版说明见 [README](../README.md)；本页保留全部细节。
...
| **放着 aespa 的歌** | 开了「听到 aespa 就跳舞」后，珉鸟会左右摇摆、上下蹦跶，暂停/静音/关掉就站好。优先走系统媒体接口；读不到时扫描播放器窗口标题（含隐藏/托盘化的窗口，主窗口关掉也能识别）并验证播放器此刻真的在出声。安静跳舞、不弹气泡 |
| **开机后第一次启动** | 自动说一声今天的日期，再查一次今日天气（仅此一次，不打扰） |
| **远程桌面 / 虚拟机** | 自动降帧（30→15fps）、默认关窗口行走，省带宽省 CPU；日志记录降级原因 |
...
（已知限制：以管理员权限运行的窗口不受普通进程的点击穿透影响，属系统 UIPI 机制。）
...
- **单文件绿色软件**：exe 直接放任意目录双击运行，无需安装、**无需管理员权限**
- 数据全部落在当前用户目录：配置/日志/提醒队列在 `%APPDATA%\MinBirdPet`，
  开机自启写 `HKCU\...\Run`（当前用户），自动查余额的计划任务也是当前用户级
- `tools/audit_portable.py` 会审计以上约束（禁止 HKLM 写入、禁止硬编码用户路径等），
  作为回归门禁的一项
